...
// ProductList component using Compound Component Pattern
...
export const ProductCartFavoriteButton = () => {
  return (
    <div className="mb-4 flex items-center justify-between gap-4 pt-6">
      <span className="me-2 rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-primary-800 dark:bg-primary-900 dark:text-primary-300">Free shipping</span>
...
export const ProductCardTitle = () => {
  const { product: { id, title } } = use(ProductCardContext);

  return (<Link href={`/product/${id}`} className="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">
    {title}
  </Link>)
}

export const ProductCardImage = () => {
  const { product: { id, title, images } } = use(ProductCardContext);
...
export const ProductCard = Object.assign(ProductCardCompoundComponent, {
  Title: ProductCardTitle,
  Image: ProductCardImage,
  Price: ProductCardPrice,
  Rating: ProductCardRating,
  Badges: ProductCardBadges,
  FavoriteButton: ProductCartFavoriteButton
})
